...
suite('Module', () => {
  ...
  test('override module 1', async () => {
    ...
    const container = module0.mergeTo(module1).build();
    const string = await container.get(key);
    expect(string).toEqual('Hola');
  });
  ...
  suite('override policy', () => {
    test('allow', () => {
      const key: BindingKey<string> = Symbol('string2s');

      Wyr.module().bind(key).toValue('asd').bind(key, 'allow').toValue('123');
      Wyr.module().bind(key).toValue('123');
    });
    test('forbid', () => {
      const key: BindingKey<string> = Symbol('string2s');
      expect(() =>
