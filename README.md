...
Provider keys can be any JavaScript `PropertyKey`: `string`, `number`, or `symbol`. Use literal keys (`as const`) or `unique symbol`s so TypeScript can track the graph precisely.
...
const app = Module({
  ...
  [db$]: toFactory([config$], async (config: { url: string }) => {
    return {
      query: async (sql: string) => ({ sql, url: config.url }),
    };
  }),
...
const repo = await app.wire(repo$);
await repo.findUser('42');
```
...
```ts
const [config, repo] = await app.wire([config$, repo$]);
```
...
For example, this graph cannot be wired without an unsafe cast because `needsDb$` depends on `db$`, but the module does not provide `db$`:
...
const broken = Module({
  [needsDb$]: toFactory([db$], (db: { query: () => unknown }) => db),
});
